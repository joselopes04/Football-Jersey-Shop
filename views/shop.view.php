...
<body class="bg-gray-50 dark:bg-gray-800">
    ...
    <div id="popularProducts" class="my-5">
        <h2 class="text-center text-xl dark:text-white mb-4">Products: <?php if (isset($numberProducts[0]->Count)) {
                                                                            echo ($numberProducts[0]->Count);
                                                                        } else {
                                                                            echo $numberOfResults;
                                                                        }
                                                                        ?></h2>
        <!-- Navigation -->
        <?php if (isset($numberProducts[0])) { ?>
            <nav class="w-full p-4 ">
                <ol class="flex h-8 space-x-2">
                    ...
                    <li class="flex items-center dark:text-white">
                        <a href="<?php echo route('league/' . $league->ID) ?>" class="flex items-center hover:underline"><?php echo $league->Name ?></a>
                    </li>
                </ol>
            </nav>
        <?php } ?>
                        ...
                        <?php foreach ($brands as $id => $brand) { ?>
                            <div class="space-y-2">
                                <div class="flex items-center ml-3 dark:text-white">
                                    ...
                                    <label class="ml-2 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2" for="category-<?php echo $id ?>"><?php if (isset($brand->Name)) {
                                                                                                                                                                                    echo ($brand->Name);
                                                                                                                                                                                } else {
                                                                                                                                                                                    echo ($names);
                                                                                                                                                                                } ?></label>
